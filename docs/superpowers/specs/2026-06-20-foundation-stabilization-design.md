...
| Supabase anon key (`sb_publishable_y16t...`) | **Present at commit `3b1de25`**, hard-coded in `lib/bootstrap.dart` |
...
**Verification:** `git log --all -S 'sb_publishable_y16t' --oneline` returns nothing; the app still initializes against Supabase using the new key.
...
3. `git log --all -S 'sb_publishable_y16t'` → empty (old anon key purged).
